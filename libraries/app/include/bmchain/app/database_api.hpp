...
struct total_block_statistic
{
    uint32_t head_block_id;
    uint32_t block_amount;
    uint32_t max_transactions;
    uint32_t min_transactions;
    uint32_t average_transactions;
...
{
   public:
      database_api(const bmchain::app::api_context& ctx);
      ~database_api();
...
FC_REFLECT_ENUM( bmchain::app::withdraw_route_type, (incoming)(outgoing)(all) );

FC_API(bmchain::app::database_api,
   // Subscriptions
