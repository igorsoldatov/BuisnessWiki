#include <bmchain/plugins/condenser_api/condenser_api.hpp>
#include <bmchain/plugins/condenser_api/condenser_api_plugin.hpp>

#include <bmchain/plugins/database_api/database_api_plugin.hpp>
#include <bmchain/plugins/block_api/block_api_plugin.hpp>
#include <bmchain/plugins/account_history_api/account_history_api_plugin.hpp>
#include <bmchain/plugins/account_by_key_api/account_by_key_api_plugin.hpp>
#include <bmchain/plugins/network_broadcast_api/network_broadcast_api_plugin.hpp>
#include <bmchain/plugins/tags_api/tags_api_plugin.hpp>
#include <bmchain/plugins/follow_api/follow_api_plugin.hpp>
#include <bmchain/plugins/reputation_api/reputation_api_plugin.hpp>
#include <bmchain/plugins/market_history_api/market_history_api_plugin.hpp>

#include <bmchain/utilities/git_revision.hpp>
...
namespace detail
{
   ...
   DEFINE_API_IMPL( condenser_api_impl, get_version )
   {
      ...
      (
         fc::string( BMCHAIN_BLOCKCHAIN_VERSION ),
   ...
   DEFINE_API_IMPL( condenser_api_impl, get_open_orders )
   {
      ...
      while( itr != idx.end() && itr->seller == owner )
      {
         ...
         // if( itr->sell_price.base.symbol == BMCHAIN_SYMBOL )
   ...
   void condenser_api_impl::set_pending_payout( discussion& d )
   {
      ...
      asset pot;
      if( _db.has_hardfork( BMCHAIN_HARDFORK_0_17__774 ) )
...
{
   if( a.voting_manabar.last_update_time < BMCHAIN_HARDFORK_0_20_TIME )
   ...
   // vp_t2 = BMCHAIN_100_PERCENT * current_mana / vests
   // vp_t1 = vp_t2 - BMCHAIN_100_PERCENT * (t2 - t1) / BMCHAIN_VOTING_MANA_REGENERATION_SECONDS
   //

   uint32_t t1 = a.last_vote_time.sec_since_epoch();
   uint32_t t2 = a.voting_manabar.last_update_time;
   uint64_t dt = (t2 > t1) ? (t2 - t1) : 0;
   uint64_t vp_dt = BMCHAIN_100_PERCENT * dt / BMCHAIN_VOTING_MANA_REGENERATION_SECONDS;

   uint128_t vp_t2 = BMCHAIN_100_PERCENT;
   vp_t2 *= a.voting_manabar.current_mana;
   vp_t2 /= vests;

   uint64_t vp_t2u = vp_t2.to_uint64();
   if( vp_t2u >= BMCHAIN_100_PERCENT )
   {
      wlog( "Truncated vp_t2u to BMCHAIN_100_PERCENT for account ${a}", ("a", a.name) );
      vp_t2u = BMCHAIN_100_PERCENT;
   }
...
{
   JSON_RPC_REGISTER_API( BMCHAIN_CONDENSER_API_PLUGIN_NAME );
}
