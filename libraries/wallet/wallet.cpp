...
#include <bmchain/account_by_key/account_by_key_api.hpp>
...
public:
   ...
   variant_object about() const
   {
      ...
      result["blockchain_version"]       = BMCHAIN_BLOCKCHAIN_VERSION;
      result["client_version"]           = client_version;
      result["bmchain_revision"]           = graphene::utilities::git_revision_sha;
      result["bmchain_revision_age"]       = fc::get_approximate_relative_time_string( fc::time_point_sec( graphene::utilities::git_revision_unix_timestamp ) );
      ...
      try
      {
         ...
         result["server_bmchain_revision"] = v.bmchain_revision;
   ...
   bool import_key(string wif_key)
   {
      ...
      bmchain::chain::public_key_type wif_pub_key = optional_private_key->get_public_key();
   ...
   int find_first_unused_derived_key_index(const fc::ecc::private_key& parent_key)
   {
      ...
      for (int key_index = 0; ; ++key_index)
      {
         ...
         bmchain::chain::public_key_type derived_public_key = derived_private_key.get_public_key();
   ...
   { try {
         ...
         bmchain::chain::public_key_type owner_pubkey = owner_privkey.get_public_key();
         bmchain::chain::public_key_type active_pubkey = active_privkey.get_public_key();
         bmchain::chain::public_key_type memo_pubkey = memo_privkey.get_public_key();
   ...
   {
      FC_ASSERT( tx_expiration_seconds < BMCHAIN_MAX_TIME_UNTIL_EXPIRATION );
   ...
   annotated_signed_transaction sign_transaction(signed_transaction tx, bool broadcast = false)
   {
      ...
      for( const public_key_type& k : minimal_signing_keys )
      {
         ...
         tx.sign( it->second, BMCHAIN_CHAIN_ID );
      }
      ...
      m["list_my_accounts"] = [](variant result, const fc::variants& a ) {
         ...
         out << "-------------------------------------------------------------------------\n";
            out << std::left << std::setw( 17 ) << "TOTAL"
                << std::right << std::setw(18) << fc::variant(total_bmt).as_string() <<" "
...
{ try {
   ...
   op.fee = asset(my->_remote_db->get_chain_properties().account_creation_fee.amount * BMCHAIN_CREATE_ACCOUNT_WITH_BMT_MODIFIER, BWC_SYMBOL );
 ...
 *  will be controlable by this wallet.
 */
annotated_signed_transaction wallet_api::create_account_delegated( string creator, asset bmt_fee, asset delegated_vests, string new_account_name, string json_meta, bool broadcast )
{ try {
   FC_ASSERT( !is_locked() );
   auto owner = suggest_brain_key();
   auto active = suggest_brain_key();
   ...
   import_key( posting.wif_priv_key );
   import_key( memo.wif_priv_key );
   return create_account_with_keys_delegated( creator, bmt_fee, delegated_vests, new_account_name, json_meta,  owner.pub_key, active.pub_key, posting.pub_key, memo.pub_key, broadcast );
} FC_CAPTURE_AND_RETHROW( (creator)(new_account_name)(json_meta) ) }
...
annotated_signed_transaction wallet_api::vote( string voter, string author, string permlink, int16_t weight, bool broadcast, string comment_bmchain )
{
   ...
   op.author = author;
   op.permlink = permlink;
   op.weight = weight * BMCHAIN_1_PERCENT;
...
vector< discussion > wallet_api::get_encrypted_discussions(string owner, string author, uint32_t limit) const{
   ...
   q.limit = limit;
   q.truncate_body = 1024;
   auto discussions = my->_remote_db->get_encrypted_discussions(q);
...
annotated_signed_transaction wallet_api::set_comment_options(string author, string permlink, uint16_t curation_rewards_percent, bool broadcast) const {
   ...
   op.author = author;
   op.permlink = permlink;
   op.max_accepted_payout = asset( 1000000000, BWC_SYMBOL );
