...
class wallet_api
{
   public:
      ...
      /**
       * Transfer funds from one account to another. BMT and SBD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       * @param amount The funds being transferred. i.e. "100.000 BMT"
      ...
      /**
       * Transfer funds from one account to another using escrow. BMT and SBD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
      ...
      * @param savings The amount of VESTS to withdraw over the next two years. Each week (amount/104) shares are
      *    withdrawn and deposited back as STEEM. i.e. "10.000000 VESTS"
      * @param broadcast true if you wish to broadcast the transaction
      */
      annotated_signed_transaction withdraw_savings( string from, asset savings, bool broadcast = false );
      ...
      */
       ...
       * to process conversion requests from SBD to BMT.
       *
      ...
      /**
       * Vote on a comment to be paid BMT
       *
       ...
       * BMT. Until the challenged proves their active key, all posting rights are revoked.
       *
      ...
      annotated_signed_transaction setup_emissions( string control_account, asset symbol, fc::time_point schedule_time,
...
FC_REFLECT( bmchain::wallet::wallet_data,
            (cipher_keys)
            (ws_server)
            (ws_user)
            (ws_password)
...
FC_REFLECT( bmchain::wallet::brain_key_info, (brain_priv_key)(wif_priv_key) (pub_key))

FC_REFLECT( bmchain::wallet::plain_keys, (checksum)(keys) )

FC_REFLECT_ENUM( bmchain::wallet::authority_type, (owner)(active)(posting) )

FC_API( bmchain::wallet::wallet_api,
        /// wallet api
        (help)(gethelp)
