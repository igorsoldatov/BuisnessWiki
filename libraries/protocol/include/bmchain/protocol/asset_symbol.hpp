...
#define BWC_SYMBOL_U64    (uint64_t('T') | (uint64_t('E') << 8) | (uint64_t('S') << 16) | (uint64_t('T') << 24) | (uint64_t('S') << 32))
...
#define VESTS_SYMBOL_U64  (uint64_t('V') | (uint64_t('E') << 8) | (uint64_t('S') << 16) | (uint64_t('T') << 24) | (uint64_t('S') << 32))
#define BWC_SYMBOL_U64    (uint64_t('B') | (uint64_t('W') << 8) | (uint64_t('C') << 16))
...
#define BWC_SYMBOL_SER    (uint64_t(3) | (BWC_SYMBOL_U64 << 8))   ///< BWC|TESTS with 3 digits of precision
#define SBD_SYMBOL_SER    (uint64_t(3) |   (SBD_SYMBOL_U64 << 8)) ///< SBD|TBD with 3 digits of precision
#define REP_SYMBOL_SER    (uint64_t(3) |   (REP_SYMBOL_U64 << 8)) ///< REP|REP with 3 digits of precision
...
template< typename Stream >
inline void unpack( Stream& s, bmchain::protocol::asset_symbol_type& sym, uint32_t )
{
   uint64_t ser = 0;
   s.read( (char*) &ser, 4 );
