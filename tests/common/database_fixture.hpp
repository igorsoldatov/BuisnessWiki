...
#define PUSH_BLOCK \
   bmchain::chain::test::_push_block
...
{ \
   const auto temp = op.field; \
   op.field = value; \
   BMCHAIN_REQUIRE_THROW( op.validate(), exc_type ); \
   op.field = temp; \
}
...
{ \
   ...
   trx.operations.back() = op; \
   op.field = bak; \
   BMCHAIN_REQUIRE_THROW(db.push_transaction(trx, ~0), exc_type); \
}
...
//   chain::database* db = nullptr;
   ...
   std::shared_ptr< bmchain::plugin::debug_node::debug_node_plugin > db_plugin;
