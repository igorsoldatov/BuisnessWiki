...
clean_database_fixture::clean_database_fixture()
{
   ...
   auto wit_plugin = app.register_plugin< bmchain::witness::witness_plugin >();
   ...
   generate_block();
   db.set_hardfork( BMCHAIN_NUM_HARDFORKS );
   generate_block();
...
void database_fixture::generate_blocks(fc::time_point_sec timestamp, bool miss_intermediate_blocks)
{
   ...
   BOOST_REQUIRE( ( db.head_block_time() - timestamp ).to_seconds() < BMCHAIN_BLOCK_INTERVAL );
}
...
)
{
   try
   {
      return account_create(
         name,
         BMCHAIN_INIT_MINER_NAME,
         init_account_priv_key,
         std::max( db.get_witness_schedule_object().median_props.account_creation_fee.amount * BMCHAIN_CREATE_ACCOUNT_WITH_BMT_MODIFIER, share_type( 100 ) ),
         key,
         post_key,
         "" );
   }
   ...
   )
{
   try
   {
      transfer( BMCHAIN_INIT_MINER_NAME, account_name, asset( amount, BWC_SYMBOL ) );
...
void database_fixture::vest( const string& from, const share_type& amount )
{
   try
   {
      ...
      op.from = from;
      op.to = "";
      op.amount = asset( amount, BWC_SYMBOL );
...
void database_fixture::set_price_feed( const price& new_price )
{
   try
   {
      for ( int i = 1; i < 8; i++ )
      {
         ...
         op.publisher = BMCHAIN_INIT_MINER_NAME + fc::to_string( i );
   ...
   generate_blocks( BMCHAIN_BLOCKS_PER_HOUR );
   BOOST_REQUIRE(
#ifdef IS_TEST_NET
...
vector< operation > database_fixture::get_last_operations( uint32_t num_ops )
{
   ...
   {
      itr--;
      ops.push_back( fc::raw::unpack< bmchain::chain::operation >( db.get(itr->op).serialized_op ) );
   }
